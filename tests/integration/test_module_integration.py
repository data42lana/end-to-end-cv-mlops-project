...
def test_src_module_pipeline(example_config, train_df, val_df, tmp_path):
    ...
    # Result
    prepared_train_df, prepared_test_df = [pd.read_csv(test_project_path.joinpath('tmp/' + f)) for f in ['train.csv', 'val.csv']]
    ...
    assert client.get_metric_history(client.search_runs([exp_id], "attributes.run_name='test-fine-tuning'")[0].info.run_id, 'f_beta')
